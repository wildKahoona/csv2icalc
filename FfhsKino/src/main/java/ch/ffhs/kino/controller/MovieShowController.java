 ...
 * 
 * Man kann entweder Plätze aus dem Kinosaal direkt auswählen oder via Funktion
 * hinzufügen (es wird dann jeweils der nächstbeste Sitz automatisch ausgewählt)
 * 
 * Tickets können einzeln oder alle aufeinmal gelöscht werden. Wenn ein Platz
 * deselektiert wird, wird auch das entsprechende Ticket gelöscht.
 * 
...
public class MovieShowController {
	...
	private void setTitle() {
		...
		lbMovieShow.setText(movieTitle + " (" + movieLanguage + "), " + fmt.format(movieShow.getDate()) + ", "
				+ getHall().getHallName());
	}
	...
	protected void addTicket(ActionEvent event) throws IOException {
		...
		while (!foundIt) {
			...
			if (isAvailable(bestSeat)) {
				foundIt = true;
				System.out.println(
						"Reihe " + bestSeat.getSeat().getSeatRow() + "Platz " + bestSeat.getSeat().getSeatColumn());
			}
		}
